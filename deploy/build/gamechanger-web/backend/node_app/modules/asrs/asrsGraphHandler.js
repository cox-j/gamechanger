...
class asrsGraphHandler extends GraphHandler { //
	constructor(opts = {}) {
		const {
	...
	async searchHelper(req, userId) {
		...
		try {
			...
			this.logger.info("asrsGraphHandler.js - line 69");
			this.logger.info(JSON.stringify(esQuery));
			let clientObj = this.searchUtility.getESClient(cloneData.clone_name, permissions);
			this.logger.info(JSON.stringify(clientObj.esClientName));
			this.logger.info(JSON.stringify(clientObj.esIndex));
			this.logger.info(JSON.stringify(userId));
			this.logger.info(JSON.stringify(cloneData));
			this.logger.info(JSON.stringify(permissions));
			this.logger.info("asrsGraphHandler.js - line 78");
			let esResults = await this.dataLibrary.queryElasticSearch(clientObj.esClientName, "gamechanger", esQuery, userId);
			// let esResults = await this.dataLibrary.queryElasticSearch(clientObj.esClientName, "gamechanger-asrs", esQuery, userId);
			// let esResults = await this.dataLibrary.queryElasticSearch(clientObj.esClientName, clientObj.esIndex, esQuery, userId);
			this.logger.info("asrsGraphHandler.js - line 82");
			...
			this.logger.info(JSON.stringify(searchResults));
			// const gT1 = new Date().getTime();

			// const { docIds, pubIds, searchTerms } = searchResults;
			const { docs, searchTerms, totalCount } = searchResults;
			this.logger.info("asrsGraphHandler.js - line 90");
			this.logger.info(JSON.stringify(searchResults.docs));
			this.logger.info(searchResults.totalCount);
			this.logger.info(JSON.stringify(this.constants.GRAPH_CONFIG.PULL_NODES_FROM_NEO4J_MAX_LIMIT));
			let results = {};
			let query = '';
			let params = [];
			let limit;

			if (totalCount <= 10000) { //this.constants.GRAPH_CONFIG.PULL_NODES_FROM_NEO4J_MAX_LIMIT) { //IF TOTAL NODES RETURNED BELOW LIMIT, WE'RE FINE --- ELSE, SEE BELOW
				// pull nodes from neo4j (this is slow, hence the limit)
				//UPDATE THE CYPHER QUERY HERE!!!!
				this.logger.info("asrsGraphHandler.js - line 100");
				const docIds = searchResults.docs.map((doc) => doc.doc_num); //doc_num);
				this.logger.info("asrsGraphHandler.js - line 102");
				this.logger.info(docIds);


				// [results, query, params] = await this.getGraphData(
				// 	`MATCH (t:Taxonomy)-[r]-(d:Document) WHERE d.Document IN $ids RETURN *;`,
				// 	{ids: docIds}, isTest, userId
				// ); //IN RETURN STATEMENT... ENSURE WE'RE RETURNING EVERYTING WE WANT TO!!!, E.G.... MATCH (s:Subject)-[r]->(node) WHERE s.NATIONAL_ID in $ids RETURN *;
				// `MATCH (a1:AIRCRAFT)-[r1]-(d:Document)
				// MATCH (a2:ASSESSMENTS)-[r2]-(d:Document)
				// MATCH (e1:ENVIRONMENT)-[r3]-(d:Document)
				// MATCH (e2:EVENTS)-[r4]-(d:Document)
				// MATCH (p:PLACE)-[r5]-(d:Document)
				// MATCH (s:\`SUMMARY TOPIC\`)-[r6]-(d:Document)
				// MATCH (t:\`TEXT TOPIC\`)-[r7]-(d:Document)
				// WHERE d.Document IN $ids RETURN *;`


				[results, query, params] = await this.getGraphData(
					`MATCH (a:ASSESSMENTS)-[r2]-(d:Document)
					MATCH (e:EVENTS)-[r4]-(d:Document)
					MATCH (k:KEYWORD)-[r7]-(d:Document)
					MATCH (s:\`SUMMARY TOPIC\`)-[r6]-(d:Document)
					WHERE d.Document IN $ids RETURN *;`,
					{ids: docIds}, isTest, userId
				); //IN RETURN STATEMENT... ENSURE WE'RE RETURNING EVERYTING WE WANT TO!!!, E.G.... MATCH (s:Subject)-[r]->(node) WHERE s.NATIONAL_ID in $ids RETURN *;
				...
				this.logger.info("asrsGraphHandler.js - line 105")
			...
			logger.info("asrsGraphHandler.js - line 149")
			const graphData = { graphData: results, searchTerms };
			logger.info("asrsGraphHandler.js - line 151")
			this.addDocIdsToPubs(graphData.graphData, userId);
			logger.info("asrsGraphHandler.js - line 153")
			...
			logger.info(JSON.stringify(graphData))
			return graphData;
	...
	async documentSearchUsingDocId(req, userId) {
		try {
			...
			const esResults = await this.dataLibrary.queryElasticSearch(clientObj.esClientName, clientObj.esIndex, esQuery);
			this.logger.info("asrsGraphHandler.js - row 743")
			this.logger.info(JSON.stringify(esResults))
...
module.exports = asrsGraphHandler; //
