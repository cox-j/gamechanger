...
const ZOOM_LIMIT = 8;
const HIDDEN_NODE_ALPHA = 1; //0.08;
const NODE_ALPHA = 1;
...
const makeGraphCollections = (graph) => {
	...
	nodes.forEach((node) => {
		const displayOrg = node['display_org_s']
			? node['display_org_s']
			: 'Uncategorized';
		const displayType = node['Document'] //WAS doc_type
			? node['Document']
			: 'Document';

		switch (node.label) {
			// case 'Taxonomy':
			// 	if (docOrgNumbers.hasOwnProperty('Taxonomy')) {
			// 		docOrgNumbers['Taxonomy'] += 1;
			// 	} else {
			// 		docOrgNumbers['Taxonomy'] = 1;
			// 	}

			// 	if (!collections.hasOwnProperty('Taxonomy')) {
			// 		collections['Taxonomy'] = [];
			// 	}

			// 	collections['Taxonomy'].push(node);
			// 	break;
			case 'AIRCRAFT':
				if (docOrgNumbers.hasOwnProperty('AIRCRAFT')) {
					docOrgNumbers['AIRCRAFT'] += 1;
				} else {
					docOrgNumbers['AIRCRAFT'] = 1;
				}

				if (!collections.hasOwnProperty('AIRCRAFT')) {
					collections['AIRCRAFT'] = [];
				}

				collections['AIRCRAFT'].push(node);
				break;

			case 'ASSESSMENTS':
				if (docOrgNumbers.hasOwnProperty('ASSESSMENTS')) {
					docOrgNumbers['ASSESSMENTS'] += 1;
				} else {
					docOrgNumbers['ASSESSMENTS'] = 1;
				}

				if (!collections.hasOwnProperty('ASSESSMENTS')) {
					collections['ASSESSMENTS'] = [];
				}

				collections['ASSESSMENTS'].push(node);
				break;
			case 'ENVIRONMENT':
				if (docOrgNumbers.hasOwnProperty('ENVIRONMENT')) {
					docOrgNumbers['ENVIRONMENT'] += 1;
				} else {
					docOrgNumbers['ENVIRONMENT'] = 1;
				}

				if (!collections.hasOwnProperty('ENVIRONMENT')) {
					collections['ENVIRONMENT'] = [];
				}

				collections['ENVIRONMENT'].push(node);
				break;
			case 'EVENTS':
				if (docOrgNumbers.hasOwnProperty('EVENTS')) {
					docOrgNumbers['EVENTS'] += 1;
				} else {
					docOrgNumbers['EVENTS'] = 1;
				}

				if (!collections.hasOwnProperty('EVENTS')) {
					collections['EVENTS'] = [];
				}

				collections['EVENTS'].push(node);
				break;
			case 'PLACE':
				if (docOrgNumbers.hasOwnProperty('PLACE')) {
					docOrgNumbers['PLACE'] += 1;
				} else {
					docOrgNumbers['PLACE'] = 1;
				}

				if (!collections.hasOwnProperty('PLACE')) {
					collections['PLACE'] = [];
				}

				collections['PLACE'].push(node);
				break;
			case 'KEYWORD':
				if (docOrgNumbers.hasOwnProperty('KEYWORD')) {
					docOrgNumbers['KEYWORD'] += 1;
				} else {
					docOrgNumbers['KEYWORD'] = 1;
				}

				if (!collections.hasOwnProperty('KEYWORD')) {
					collections['KEYWORD'] = [];
				}

				collections['KEYWORD'].push(node);
				break;
			case 'Document':
				...
				if (!collections.hasOwnProperty(`${node.doc_type} ${node.doc_num}`)) {
					...
					collections[`${node.doc_num}`] = [];
				}

				collections[`${node.doc_type} ${node.doc_num}`].push(node);
				collections[`${node.doc_num}`].push(node);
				break;
			default:
				break;
		}
...
export default function ASRSGraphView(props) {
	const {
		width,
		height,
		graphData,
	...
	const [nodeRelSize, setNodeRelSize] = React.useState(1);
	...
	///THIS IS WHERE ALL FUNCTIONS ARE CALLED
	useEffect(() => {
	...
	useEffect(() => { //CHANGE THIS UP FOR ASRS
		...
		if (communityView) {
			...
			graph.nodes.forEach((node) => { //THIS IS WHERE WE SPECIFY DIFFERENT NODE TYPES & DISPLAY CONFIG
				const displayOrg = node['display_org_s']
					? node['display_org_s']
					: 'Uncategorized';
				...
				switch (node.label) {
					case 'Document':
						if (!tmpLegendData['Document']) { //DEFINE COLOR + NAME IN LEGEND, ETC...
							tmpLegendData['Document'] = {
								color: typeColorMap.document,
								name: 'Document',
							};
						}
						node.color = typeColorMap.document;
						break;
					// case 'Taxonomy':
					// 	if (!tmpLegendData['Taxonomy']) {
					// 		tmpLegendData['Taxonomy'] = {
					// 			color: typeColorMap.taxonomy,
					// 			name: 'Taxonomy',
					// 		};
					// 	}
					// 	node.color = typeColorMap.taxonomy;
					// 	break;
					case 'AIRCRAFT':
						if (!tmpLegendData['AIRCRAFT']) {
							tmpLegendData['AIRCRAFT'] = {
								color: typeColorMap.AIRCRAFT,
								name: 'AIRCRAFT',
							};
						}
						node.color = typeColorMap.AIRCRAFT;
						break;
					case 'KEYWORD':
						if (!tmpLegendData['KEYWORD']) {
							tmpLegendData['KEYWORD'] = {
								color: typeColorMap.KEYWORD,
								name: 'KEYWORD',
							};
						}
						node.color = typeColorMap.KEYWORD;
						break;
					case 'ASSESSMENTS':
						if (!tmpLegendData['ASSESSMENTS']) {
							tmpLegendData['ASSESSMENTS'] = {
								color: typeColorMap.ASSESSMENTS,
								name: 'ASSESSMENTS',
							};
						}
						node.color = typeColorMap.ASSESSMENTS;
						break;
					case 'ENVIRONMENT':
						if (!tmpLegendData['ENVIRONMENT']) {
							tmpLegendData['ENVIRONMENT'] = {
								color: typeColorMap.ENVIRONMENT,
								name: 'ENVIRONMENT',
							};
						}
						node.color = typeColorMap.ENVIRONMENT;
						break;
					case 'EVENTS':
						if (!tmpLegendData['EVENTS']) {
							tmpLegendData['EVENTS'] = {
								color: typeColorMap.EVENTS,
								name: 'EVENTS',
							};
						}
						node.color = typeColorMap.EVENTS;
						break;
					case 'PLACE':
						if (!tmpLegendData['PLACE']) {
							tmpLegendData['PLACE'] = {
								color: typeColorMap.PLACE,
								name: 'PLACE',
							};
						}
						node.color = typeColorMap.PLACE;
						break;
					case 'TEXT TOPIC':
						if (!tmpLegendData['TEXT TOPIC']) {
							tmpLegendData['TEXT TOPIC'] = {
								color: typeColorMap.TEXT_TOPIC,
								name: 'TEXT TOPIC',
							};
						}
						node.color = typeColorMap.TEXT_TOPIC;
						break;
					case 'SUMMARY TOPIC':
						if (!tmpLegendData['SUMMARY TOPIC']) {
							tmpLegendData['SUMMARY TOPIC'] = {
								color: typeColorMap.SUMMARY_TOPIC,
								name: 'SUMMARY TOPIC',
							};
						}
						node.color = typeColorMap.SUMMARY_TOPIC;
						break;
					case 'COMPONENT':
						if (!tmpLegendData['COMPONENT']) {
							tmpLegendData['COMPONENT'] = {
								color: typeColorMap.COMPONENT,
								name: 'COMPONENT',
							};
						}
						node.color = typeColorMap.COMPONENT;
						break;
					case 'UKN_Document':
	...
	const showNodeContextMenu = () => { //CUSTOMIZE SELECTOR ENABLING USER TO SPECIFY RELATIONSHIP VIEW BETWEEN NODES
		const myStyle = {
			position: 'absolute',
			top: 'calc(50% - 130px)',
			left: 'calc(50% - 130px)',
		...
		switch (nodeLabel) {
			...
			case 'KEYWORD':
				...
				break;
			case 'ASSESSMENTS':
				cardText = 'ASSESSMENTS';
				break;
			case 'ENVIRONMENT':
				cardText = 'ENVIRONMENT';
				break;
			case 'EVENTS':
				cardText = 'EVENTS';
				break;
		...
		if (nodeLabel === 'Document' && !notInOriginal) {
			...
			if (Config.GAMECHANGER.SHOW_TAXONOMIES) {
				menuItems.push({
					className: 'fa fa-lightbulb-o fa-2x',
					onClick: () =>
						handleContextMenuButtonClicked(
							() => showTaxonomiesForNode(),
							true,
							'showTaxonomies'
						),
					tooltip: 'Display taxonomy nodes for this document',
				});
			}
		}
	...
	const handleShowGraphCard = async () => {
		...
		const ignoreLabels = ['Publication', 'Entity', 'Topic', 'Taxonomy', 'AIRCRAFT', 'KEYWORD', 'ASSESSMENTS', 'ENVIRONMENT', 'EVENTS', 'PLACE','SUMMARY TOPIC', 'COMPONENT', 'TEXT TOPIC'];
		if (!ignoreLabels.includes(node?.label)) {
			...
			console.log("asrsGraphView.js - row 889");
	...
	const showReferencesForNode = async (isUKN = false) => {
		...
		const resp = await gameChangerAPI.callGraphFunction({ //THIS IS WHERE BACKEND FUNCTIONS (asrsGraphHandler.js) ARE INVOKED
			functionName: 'searchHelper',
			cloneName: cloneData.clone_name,
			options: {
				ref_name: refName,
				isUnknown: isUKN,
	...
	const showTaxonomiesForNode = () => {
		...
		gameChangerAPI
			.callGraphFunction({
				functionName: 'getTaxonomiesForNode',
				cloneName: cloneData.clone_name,
				options: {
					doc_id: node.Document,
				},
	...
	const create2dGraphNode = (node, ctx, globalScale) => {
		...
		// const combinedTypes = ['Topic', 'Entity', 'UKN_Document']; //REPLACE WITH MY NODE TYPES
		// const combinedTypes = ['Taxonomy', 'Document']; //REPLACE WITH MY NODE TYPES FOR ASRS
		const combinedTypes = ['AIRCRAFT', 'KEYWORD', 'ASSESSMENTS', 'ENVIRONMENT', 'EVENTS', 'PLACE', 'Document', 'SUMMARY TOPIC', 'TEXT TOPIC']; //REPLACE WITH MY NODE TYPES FOR ASRS
	...
	const createNodeLabel = (node) => {
		// const useNameOnly = ['Entity', 'Topic', 'UKN_Document']; //REPLACE THIS WITH MY NODE TYPES
		// const useNameOnly = ['Taxonomy', 'Document']; //REPLACE THIS WITH MY NODE TYPES
		const useNameOnly = ['Document']; //'AIRCRAFT', 'ASSESSMENTS', 'ENVIRONMENT', 'EVENTS', 'PLACE']; //, 'Document']; //REPLACE THIS WITH MY NODE TYPES
	...
	// 	// const useNameOnly = ['Entity', 'Topic', 'UKN_Document']; //REPLACE THIS WITH MY NODE TYPES
	// 	// const useNameOnly = ['Taxonomy', 'Document']; //REPLACE THIS WITH MY NODE TYPES
	// 	const useNameOnly = ['AIRCRAFT', 'ASSESSMENTS', 'ENVIRONMENT', 'EVENTS', 'PLACE', 'Document']; //REPLACE THIS WITH MY NODE TYPES
	// 	return useNameOnly.includes(node.label)
	// 		? node.name
	// 		: `${node.doc_type} ${node.doc_num}`;
	// 	switch (node.label) {
	...
	const createNodeTooltip = (node) => {
		// const useNameOnly = ['Entity', 'Topic', 'UKN_Document']; //REPLACE THIS WITH MY NODE TYPES
		const useNameOnly = ['AIRCRAFT', 'KEYWORD', 'ASSESSMENTS', 'ENVIRONMENT', 'EVENTS', 'PLACE', 'Document', 'SUMMARY TOPIC', 'TEXT TOPIC', 'COMPONENT']; //REPLACE THIS WITH MY NODE TYPES
	...
	console.log("asrs Graph View row 2020");
...
export const MemoizedASRSGraphView = React.memo(ASRSGraphView);
