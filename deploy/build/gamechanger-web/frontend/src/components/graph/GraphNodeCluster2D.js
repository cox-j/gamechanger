...
export default function GraphNodeCluster2D(props) {
	...
	const [chargeStrength, setChargeStrength] = React.useState(-10);
	const [linkDistance, setLinkDistance] = React.useState(125);
	const [linkIterations, setLinkIterations] = React.useState(1);
	const [nodeRelativeSize, setNodeRelativeSize] = React.useState(1);
	const [zoomFactor, setZoomFactor] = React.useState(0.5);
	const [nodeLabelColors, setNodeLabelColors] = React.useState({});
						...
						<div style={{ margin: '16px 0px 0px' }}>
							...
							>
								{nodesInGroup.map((node) => {
									...
									let isNotDocumentNode = node.label === 'AIRCRAFT' || node.label === 'ASSESSMENTS' || node.label === 'ENVIRONMENT' || node.label === 'EVENTS' || node.label === 'PLACE' || node.label === 'SUMMARY TOPIC' || node.label === 'TEXT TOPIC' || node.label === 'COMPONENT' || node.label === 'KEYWORD';
									return (
										<GCTooltip
											title={(isNotDocumentNode || node.label === 'UKN_Document') ? '' : node.display_title_s}
											arrow
											style={{ zIndex: 99999 }}
										>
											<StyledNodeGroupNode
											...
											>
												{isNotDocumentNode ? node.name : `${node.doc_type} ${node.doc_num}`}
											</StyledNodeGroupNode>
										</GCTooltip>
									);
								})}
