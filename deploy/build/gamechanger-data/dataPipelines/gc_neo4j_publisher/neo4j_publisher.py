...
class Neo4jPublisher:
    ...
    def process_json(self, filepath: str, q: mp.Queue) -> str:
        with open(filepath) as f:
            ...
            o["taxonomies"] = j.get("taxonomies", "")
            o["summary_topics"] = j.get("summary_topics", "")
            o["text_topics"] = j.get("text_topics", "")
            o["keyw_5"] = j.get("keyw_5", "")

            print(o["doc_num"])
            print(o["keyw_5"])
            process_query('CALL policy.createASRSTaxonomyNodesFromJson(' + json.dumps(json.dumps(o)) + ')')             
    ...
    def process_airport_relationships(self) -> None:
        process_query(
            'MATCH'
            +'(s:Subject),'
            +'(a:Airport)'
            + 'WHERE s.ARRIVAL_AIRPORT = a.name'
            + 'CREATE (s)-[:FLYING_INTO]->(a);'
        )
        process_query(
            'MATCH'
            +'(s:Subject),'
            +'(a:Airport)'
            + 'WHERE s.DEPARTURE_AIRPORT = a.name'
            + 'CREATE (s)-[:FLYING_OUT_OF]->(a);'
        )
        return