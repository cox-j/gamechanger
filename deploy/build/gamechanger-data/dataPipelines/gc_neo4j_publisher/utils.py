...
from .neo4j_publisher_im import Neo4jPublisherIM
import os
import sys
import math
...
class Neo4jJobManager:
    ...
    @staticmethod
    def process_files_im(files: t.List[str], file_dir: str, q: mp.Queue, publisher: Neo4jPublisherIM, max_threads: int) -> None:
        publisher.process_dir(files, file_dir, q, max_threads)
    ...
    def run_update_asrs(self, source: t.Union[str, Path], max_threads: int) -> None:
        """Run Neo4j Update Job
        :param source: Path to source directory
        :param max_threads: max threads to use
        """
    ...
    def run_update_im(self, source: t.Union[str, Path], max_threads: int) -> None:
        """Run Neo4j Update Job
        :param source: Path to source directory
        :param max_threads: max threads to use
        """
        ...
        publisher = Neo4jPublisherIM() ######CHANGE THIS

        q = mp.Queue()
        proc = mp.Process(target=self.listener, args=(q, len(files)))
        proc.start()
        workers = [mp.Process(target=self.process_files_im, args=(file_chunks[i], file_dir, q, publisher, max_threads)) for i in range(n)]
        for worker in workers:
            worker.start()
        for worker in workers:
            worker.join()
