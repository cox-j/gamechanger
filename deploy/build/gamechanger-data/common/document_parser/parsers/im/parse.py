    ...
    if file_extension == '.pdf':
        ...
        for count,line in enumerate(open(str(f_name), 'r', encoding='utf-8-sig')):
            ...
            try:
                ...
                doc_dict["filename"] = row["title"]
                doc_dict["filename_clean"] = row["title"]
                doc_dict["doc_num"] = row["title"]

                doc_dict["doc_type"] = "Tearline.mil"
                doc_dict["doc_date"] = "2022"
                doc_dict["authors"] = ', '.join(row["authors"])
                doc_dict["categories"] = row["categories"]
                doc_dict["categories_str"] = ', '.join(row["categories"])
                doc_dict["tags"] = row["tags"]
                doc_dict["tags_str"] = ', '.join(row["tags"])
                doc_dict["url"] = row["url"]
