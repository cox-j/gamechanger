...
def process_dir(
    ...
    files = [x for x in p if x.is_file() and (str(x).endswith("pdf") or str(x).endswith("html") or str(x).endswith("csv") or str(x).endswith("json")
        or (filetype.guess(str(x)) is not None and (filetype.guess(str(x)).mime == "pdf" or filetype.guess(str(x)).mime == "csv" or filetype.guess(str(x)).mime == "json")))]
